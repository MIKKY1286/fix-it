...
import { FiHome, FiBriefcase, FiStar, FiCalendar, FiCreditCard, FiFolder, FiUser, FiSettings, FiBell, FiLogOut, FiMenu, FiX, FiMessageSquare } from 'react-icons/fi';
...
const ArtisanLayout = () => {
  ...
  const menuItems = [
    { name: 'Dashboard', path: '/artisan', icon: <FiHome size={18} /> },
    { name: 'Job Manager', path: '/artisan/jobs', icon: <FiBriefcase size={18} /> },
    { name: 'Calendar', path: '/artisan/calendar', icon: <FiCalendar size={18} /> },
    { name: 'Wallet & Payouts', path: '/artisan/wallet', icon: <FiCreditCard size={18} /> },
    { name: 'Portfolio', path: '/artisan/portfolio', icon: <FiFolder size={18} /> },
    { name: 'Reviews', path: '/artisan/reviews', icon: <FiStar size={18} /> },
    { name: 'Messages', path: '/artisan/messages', icon: <FiMessageSquare size={18} /> },
    { name: 'Business Profile', path: '/artisan/profile', icon: <FiUser size={18} /> },
    { name: 'Settings', path: '/artisan/settings', icon: <FiSettings size={18} /> },
  ];
  ...
  const getPageTitle = () => {
    ...
    return activeItem ? activeItem.name : 'Artisan Workspace';
  };
    ...
    <div className="flex min-h-screen bg-slate-50">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-secondary text-white shrink-0 border-r border-secondary-light/20 relative z-30">
        ...
        <div className="px-8 py-6 border-b border-white/5 flex items-center gap-2.5">
          ...
          <span className="text-[10px] uppercase font-bold tracking-widest bg-accent-light text-accent px-2 py-0.5 rounded-md">
            Artisan
          </span>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 bg-white border-b border-secondary/5 flex items-center justify-between px-4 sm:px-6 lg:px-8 relative z-25">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Live Status Badge */}
            <span className="hidden md:inline-flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-bold text-accent bg-accent-light rounded-full uppercase tracking-wider">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              ...
              >
                <div className="h-9 w-9 rounded-full bg-accent-light text-accent border border-accent/20 flex items-center justify-center font-bold text-sm">
                    ...
                    >
                      <div className="px-4 py-3.5 border-b border-secondary/5">
                        <div className="flex items-center gap-1">
                          <p className="text-xs font-semibold text-secondary/40 uppercase tracking-wider">Workspace</p>
                          <GoVerified size={10} className="text-accent" />
                        </div>
                        <p className="text-sm font-bold text-secondary mt-0.5 truncate">{userProfile?.name || 'Verified Artisan'}</p>
                      </div>
                      <div className="py-1">
                        <Link
                          to="/artisan/profile"
                        ...
                        >
                          <FiUser size={16} /> Business Profile
                        </Link>
                        <Link
                          to="/artisan/settings"
                          onClick={() => setShowUserDropdown(false)}
