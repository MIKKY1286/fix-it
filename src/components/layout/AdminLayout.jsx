...
import { FiHome, FiUsers, FiCheckSquare, FiGrid, FiFileText, FiDollarSign, FiAlertTriangle, FiSettings, FiBell, FiLogOut, FiMenu, FiX } from 'react-icons/fi';
...
const AdminLayout = () => {
  ...
  const menuItems = [
    ...
    { name: 'User Management', path: '/admin/users', icon: <FiUsers size={18} /> },
    { name: 'Verification', path: '/admin/verification', icon: <FiCheckSquare size={18} /> },
    { name: 'Categories', path: '/admin/categories', icon: <FiGrid size={18} /> },
    { name: 'Bookings Log', path: '/admin/bookings', icon: <FiFileText size={18} /> },
    { name: 'Payments & Transact', path: '/admin/payments', icon: <FiDollarSign size={18} /> },
    { name: 'Disputes & Reports', path: '/admin/disputes', icon: <FiAlertTriangle size={18} /> },
    { name: 'System Settings', path: '/admin/settings', icon: <FiSettings size={18} /> },
  ];
  ...
  const getPageTitle = () => {
    ...
    return activeItem ? activeItem.name : 'Admin Terminal';
  };
    ...
    <div className="flex min-h-screen bg-slate-50">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-secondary text-white shrink-0 border-r border-secondary-light/20 relative z-30">
        ...
        <div className="px-8 py-6 border-b border-white/5 flex items-center gap-2.5">
          ...
          <span className="text-[10px] uppercase font-bold tracking-widest bg-danger-light text-danger px-2 py-0.5 rounded-md flex items-center gap-0.5">
            <MdOutlineShield size={10} />
            Admin
          </span>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 bg-white border-b border-secondary/5 flex items-center justify-between px-4 sm:px-6 lg:px-8 relative z-25">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* System Badge */}
            <span className="hidden md:inline-flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-bold text-danger bg-danger-light rounded-full uppercase tracking-wider">
              System Live
            </span>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              ...
              >
                <div className="h-9 w-9 rounded-full bg-danger-light text-danger border border-danger/20 flex items-center justify-center font-bold text-sm">
                  {userProfile?.avatarText || 'AD'}
                </div>
              </button>
                  ...
                  <React.Fragment>
                    ...
                    >
                      <div className="px-4 py-3.5 border-b border-secondary/5">
                        <p className="text-xs font-semibold text-secondary/40 uppercase tracking-wider">Session</p>
                        <p className="text-sm font-bold text-secondary mt-0.5 truncate">{userProfile?.name || 'Super Admin'}</p>
                      </div>
                      <div className="py-1">
                        <Link
                          to="/admin/settings"
